...
/**
 *class that uses the wrapper facade pattern in order to define a factory that creates new SOCKStreams passively.
 */
public class SOCKAcceptor
{
	 ...
	 */
	public void accept (SOCStream sockStream) throws SocketException, IOException
	{
		// Block in accept. Returns when a connection shows up and sets
		// the streams
		sockStream.socket (this.listenSocket_.accept ());
		System.out.println("Accepted connection from " +		 sockStream.socket ().getInetAddress ());
	}
